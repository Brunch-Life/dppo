...
def parse_args():
    ...
    parser.add_argument(
        "--val_data_dir", type=str, default="/mnt/iag/user/zhangxin/shangyu/challenge-0308/data/val_v1.1",
...
class GenieEvaluator:
    ...
    def predict_zframe_logits(self, input_ids: torch.LongTensor,actions) -> tuple[torch.LongTensor, torch.FloatTensor]:
        """
        ...
        input_action = actions.reshape(-1, WINDOW_SIZE, 28).to(self.device)

        all_samples = []
        all_logits = []
        # print(inputs_THW.shape)
        for timestep in range(1, WINDOW_SIZE):
            ...
            if timestep == 1:
                samples_HW, factored_logits = self.model.maskgit_generate(
                    inputs_masked, input_action, out_t=timestep, maskgit_steps=self.args.maskgit_steps,
