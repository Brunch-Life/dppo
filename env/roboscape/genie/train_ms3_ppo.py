...
class Args:
    ...
    # vla
    vla_path: str = "/iag_ad_01/ad/tangyinzhou/tyz/WM4RL/openvla-7b"
...
class Runner:
    def __init__(self, all_args: Args):
        ...
        # env
        unnorm_state = self.policy.vla.get_action_stats(
            self.args.vla_unnorm_key
        )  # what is this?
        # self.env = SimlerWrapper(self.args, unnorm_state)
        self.env = WMEnvWrapper(self.args, unnorm_state)
    ...
    def collect(self):
        ...
        obs_image = self.buffer.obs[self.buffer.step]  # B, H, W, 3
    ...
    def run(self):
        ...
        for episode in range(max_episodes):
            ...
            for _ in tqdm(range(self.args.episode_len), desc="rollout"):
                ...
                # action = torch.concat(
                #     [action, action[:, -1].unsqueeze(1)], dim=1
                # )  # NEED TO CHANGE!!!!
                obs_img, reward, done, env_info = self.env.step(action)

                data = (obs_img, action, logprob, value, reward.reshape(-1, 1), done)
                self.insert(data)
