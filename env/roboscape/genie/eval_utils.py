...
from env.roboscape.genie.genie.factorization_utils import factorize_labels
...
def decode_tokens(
    ...
    # output_path = (
    #     f"/iag_ad_01/ad/tangyinzhou/tyz/observation-genie/check_{random.randint(0,100)}"
    # )
    # os.makedirs(output_path)
    # for idx, img in enumerate(decoded_imgs):
    #     img.save(f"{output_path}/{idx}.png")  # 保存为 PNG
    decoded_tensor = torch.stack(
        [transforms_f.pil_to_tensor(pred_img) for pred_img in decoded_imgs]
    )
    return rearrange(
        decoded_tensor, "(b t) c H W -> b t c H W", b=reshaped_token_ids.size(0)
    )
...
def compute_loss(
    ...
    factored_labels = factorize_labels(
        labels_THW, num_factored_vocabs, factored_vocab_size
    )
    return (
...
def compute_lpips(
    ...
    flattened_a, flattened_b = [
        rearrange(frames / 127.5 - 1, "b t c H W -> (b t) c H W")
