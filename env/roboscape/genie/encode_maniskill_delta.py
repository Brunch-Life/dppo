...
def load_model(gpu_id):
    ...
    tokenizer = VQModel(
        VQConfig(),
        ckpt_path="/iag_ad_01/ad/tangyinzhou/tyz/observation-genie/genie/magvit2.ckpt",
    ).cuda()
    tokenizer.eval()
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for jpeg_bytes in data["observation"]["rgb"]:
            ...
            tensor = self.resize_transform(
                tensor
            )  # ← 你定义的 transforms.Resize(256,256)
            resized_frames.append(tensor)
        resized_frames = torch.stack(resized_frames)  # (T, 3, 256, 256)
        actions = data["action"]
        ...
        action_all = np.diff(action_all, axis=0, append=action_all[-1:])
        obj = " ".join(video_path.split("/")[-3].split("_")[:-2])
        tgt = video_path.split("/")[-3].split("_")[-2]
        ...
        if not resized_frames.shape[0] == action_all.shape[0]:
            print(1)
        return (
            resized_frames,
            action_all,
...
def parse_args():
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="/tangyinzhou-tos-volc-engine/tyz/encoded_maniskill_delta",
    )
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID to use")
    parser.add_argument(
        "--task", type=str, default="20250903_122339", help="each part is 1w"
    )
