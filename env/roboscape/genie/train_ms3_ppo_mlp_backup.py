...
class Runner:
    ...
    def insert(self, data):
        ...
        self.buffer.insert(
            obs_img, state, actions, logprob, value_preds, rewards, masks
        )
    ...
    def eval(self) -> dict:
        ...
        for _ in range(self.args.episode_len):
            ...
            obs = {
                k: torch.tensor(v) for k, v in obs.items()
            }  # obs: dict, image: tensor, state: tensor
            value, action, logprob = self._get_action(
                obs, deterministic=True
            )  # need tensor
            print(f"!!action: {action[0]}")

            obs_img, state, reward, terminated, truncated, env_info = (
    ...
    def render(self, epoch: int) -> dict:
        ...
        obs_img, state, info = self.env_wrapper.reset(
            eps_count=0
        )  # obs_img: np.ndarray, state: np.ndarray, info: dict
        ...
        for _ in range(self.args.episode_len):
            obs = dict(
                image=obs_img, state=state
            )  # obs: dict, image: np.ndarray, state: np.ndarray
            obs = {
                k: torch.tensor(v) for k, v in obs.items()
            }  # obs: dict, image: tensor, state: tensor
            value, action, logprob = self._get_action(
            ...
            obs_img_new, state, reward, terminated, truncated, env_info = (
                self.env_wrapper.step(action)
            )
            ...
            if self.args.render_info:
                for j in range(len(infos)):
                    images[j + 1] = visualization.put_info_on_image(
                        images[j + 1], infos[j], extras=[f"State: {state[i]}"]
                    )
