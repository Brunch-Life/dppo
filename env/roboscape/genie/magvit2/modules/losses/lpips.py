...
class vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        ...
        features_weights = {k.replace('features.', ''): v for k, v in local_weights.items() if 'features' in k}

        # 将本地权重加载到模型的features部分
        vgg_model.features.load_state_dict(features_weights)

        # 获取VGG16模型的features部分
