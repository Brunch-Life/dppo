...
def parse_args():
    ...
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default="/mnt/iag/user/zhangxin/shangyu/challenge-0308/data/train_v1.1",
    ...
    parser.add_argument("--use_target_action", action="store_true")
...
def main():
    ...
    accelerator = Accelerator(
        gradient_accumulation_steps=1, project_dir=args.output_dir
    )
    ...
    if args.load_ckpt is not None:
        model = STMaskGIT.from_pretrained(args.load_ckpt)
    ...
    collate_fn = (
        ...
        else get_maskgit_collator(config)
    )
    ...
    accelerator.init_trackers(
        project_name="1XGPT_muP_MAGVIT2_v0", config=experiment_config
    )
    ...
    loss_info = torch.zeros(3, device=accelerator.device)  # sum, count
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with ctx_manager:
                outputs = model(**batch)
                loss = outputs.loss
                loss_done = outputs.loss_done
                loss_info[0] += loss.detach() * batch_size
                loss_info[1] += batch_size
                loss_info[2] += loss_done.detach() * batch_size
            ...
            avg_train_done_loss = (loss_info[2] / loss_info[1]).item()
            ...
            logger.info(
                ...
                f"Train Done Loss:{avg_train_done_loss:.4f}, "
                f"Epoch: {epoch}, "
                f"Update Step: {completed_steps}, "
                f"Examples Processed: {completed_steps * args.per_device_train_batch_size * args.gradient_accumulation_steps * accelerator.num_processes}, "
                f"Learning Rate: {lr_scheduler.get_last_lr()[0]:.6f}, "
                f"FLOPs: {(completed_steps + 1) * experiment_config['FLOPs_per_update_step']:.2e}, "
                f"Throughput (examples/sec): {experiment_config['effective_batch_size'] / batch_time:.2f}"
            )
            ...
            if completed_steps % args.eval_every_n_steps == 0:
                ...
                for step, batch in enumerate(eval_dataloader):
                    ...
                    eval_done_losses.append(
                        accelerator.gather_for_metrics(loss_done.repeat(batch_size))
                    )
                ...
                eval_done_losses = torch.cat(eval_done_losses)
                eval_done_loss = torch.mean(eval_done_losses).item()
                ...
                logger.info(
                    ...
                    f"Eval Done Loss:{eval_done_loss:.4f}, "
                    f"Eval Teacher Accuracy: {eval_teacher_acc:.2f}, "
                    f"Epoch: {epoch}, "
                    f"Update Step: {completed_steps}, "
                    f"Examples Processed: {completed_steps * args.per_device_train_batch_size * args.gradient_accumulation_steps * accelerator.num_processes}, "
                    f"FLOPs: {completed_steps * experiment_config['FLOPs_per_update_step']:.2e}"
                )
