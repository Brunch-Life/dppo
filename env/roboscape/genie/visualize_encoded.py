        ...
        for shard_ind in trange(math.ceil(len(video_data) / batch_size)):
            ...
            if model.use_ema:
                with model.ema_scope():
                    ...
                    # 解码图像
                    decoded = model.decode(quant.to(device=device, dtype=dtype))
                    # 缩放并转换为uint8
                    rescaled = rescale_magvit_output(decoded)
                    # 将BGR转换为RGB - 这是关键的修改
                    # rgb_converted = rescaled[:, [2, 1, 0], ...]
                    rgb_converted = rescaled
                    decoded_imgs.append(rgb_converted)
...
def main():
    ...
    video_frames = decode_latents_wrapper(max_images=args.max_images)(
        video_tokens[args.offset : 60 : args.stride]
    )
